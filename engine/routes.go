...
func bindRoutes(
	...
	r.Route("/apis/web/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/listens/duplicates", handlers.GetDuplicateListensHandler(db))
			r.Get("/clean-mode/candidates", handlers.GetCleanModeCandidatesHandler(db))
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/search-images", handlers.SearchImagesHandler())
			...
			r.Post("/split/artists", handlers.SplitArtistsHandler(db))
			...
			r.Get("/live-shows", handlers.GetLiveShows(db))
			r.Get("/live-shows/{id}", handlers.GetLiveShowDetails(db))
			r.Post("/live-shows/search-setlist", handlers.SearchSetlistFm())
			r.Get("/live-shows/user-attended", handlers.GetUserAttendedSetlists())
			r.Post("/live-shows/import", handlers.ImportSetlist(db))
			r.Delete("/live-shows/{id}", handlers.DeleteLiveShow(db))
			r.Post("/live-shows/link-artist", handlers.LinkArtist(db))
